import qdarkstyle
from variables import PRIMARY_COLOR, DARKEST_PRIMARY_COLOR, DARK_PRIMAARY_COLOR, isEmpty, isNumOrDot, isValidNumber, converToNumber
...
qss = f"""
    ...
    QPushButton[class="specialButton"]:hover {{
        color: #fff;
        background: {DARK_PRIMAARY_COLOR};
    }}
...
def setupTheme(app):
    # Carrega o estilo escuro do qdarkstyle
    dark_style = qdarkstyle.load_stylesheet_pyside6()

    # Aplica o estilo escuro + o QSS personalizado
    app.setStyleSheet(dark_style + qss)
        ...
        for rowNumber, rowData in enumerate(self._gridMask):
            for colNumber, buttonText in enumerate(rowData):
                ...
                if not isNumOrDot(buttonText) and not isEmpty(buttonText):
                    button.setProperty('class', 'specialButton')
